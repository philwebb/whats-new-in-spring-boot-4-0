...
* Run `caches.sh` (on `main` and `live` branch)
...
* The `live` branch is checked out
* docker compose down
...
* Have a preview of Spring Boot 4.1
...
* Upgrade the version in the POM to `4.0.3`
...
We've got a lot of benefits, but one things we've lost:
...
[,shell]
----
java -jar ./target/isdb-count-0.0.1-SNAPSHOT.jar 
----
...
** Note the `@ImportHttpServices` to import the client to the group
* Show the records

Run the application hit http://localhost:8081 and watch it fail!

=== HTTP Service Groups

* Explain HTTP serviceclient and how it relates to groups
* Explain the `HttpServiceGroupConfigurer` interface
* Explain the property hierarchy
** `http.clients.` is global
** `http.serviceclient.<name>` is a group
...
This is saying for the "stuffie" service client group insert the version using a header.
...
(if short on time, jump to `otel` and just demo)
...
In order to log correlation IDs on the `isdb` app we can add the `spring-boot-micrometer-tracing-opentelemetry` modules to it.
We also need `io.micrometer:micrometer-tracing-bridge-otel` so that micrometer picks up correlation headers.
...
We have a new requirement to perform random stuffie audits to check inventory.
We want a simple app that randomly picks a user to audit and lists their stuffies.

Obvious there's the potential of a huge amount of data here, so management are insisting we use gRPC.
Let's take a sneak peak at what Spring Boot 4.1 can do for us.

==== Upgrade to Spring Boot 4.1

Show or add SNAPSHOT repo support:
...
Bump the parent in `spring-isdb` to `4.1.0-SNAPSHOT`

==== Add the Probuf File

The service definition proto file can be added to `src/main/proto/isdb.proto`

[,proto]
----
syntax = "proto3";

option java_package = "com.example.isdb.proto";
option java_multiple_files = true;

service Isdb {
    rpc GetStuffiesByOwner (GetStuffiesByOwnerRequest) returns (stream GetStuffiesByOwnerResponse) {
    }
}

message GetStuffiesByOwnerRequest {
    string owner = 1;
}

message GetStuffiesByOwnerResponse {
    string name = 1;
    string type = 2;
}
----

Add `protobuf-maven-plugin` and note that we've configured it with sensible defaults in the parent.
...
Dont'r refresh yet as the generated code won't compile.

==== Add Spring gRPC Server Support

There are three options for gRPC:

* Netty
* Netty (but shaded!)
* Servlet

We're going to use servlet since we already have the servlet stack in the app:
...
==== Add the gRPC Service Code

Spring gRPC will expose all `BindableService` beans.
There's also a nice `@GrpcService` serotype annotation.

Add the code to `com.example.isdb.stuffie`:
...
Show the `isdb-audit` application.

* Show the `pom.xml`
** Note the `spring-boot-starter-grpc-client`
** Note the `protobuf-maven-plugin`
* Show the proto file
* Show the `IsdbAuditApplication`
** Note the `@ImportGrpcClients`
** Show the `audit` method
